...
def interpret( all_df, df , days ):
 def parser(df,days,sname_is_last = False) : 
   ...
   pivot_filtered_splitup = pd.pivot_table(filtered,index=['Salesperson','Beat Name','party','Bill Number'],values=['O/S Amount','In Days'],aggfunc={'O/S Amount':np.sum,'In Days':np.max},margins=True)
   pivot_total_bills = pd.pivot_table(unfiltered,index=['Salesperson','Beat Name','party'],values=['Bill Number'],aggfunc={'Bill Number':pd.Series.nunique},margins=True)
 ...
 all_df = all_df.rename( columns = {"Salesperson Name" : "Salesperson",outstanding_col_name:"O/S Amount","Bill Ageing (In Days)":"In Days","Bill No" : "Bill Number"  })
